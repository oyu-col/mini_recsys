...
class RankerInference:
    ...
    def load_model(self, model_type: str, target_col, fold: int):
        model_dir = Path(
            os.environ.get("OTTO_MODEL_DIR", self.config.dir_config.exp_output_dir)
        )
        if model_type == "lgbm":
            return lgb.Booster(
        ...
        if model_type == "catboost":
            if CatBoost is None:
                raise ImportError("catboost is required for catboost inference")
            model = CatBoost()
            model.load_model(model_dir / f"catboost_{target_col}_fold{fold}.txt")
            return model
        if model_type == "catboost_ranker":
            if CatBoost is None:
                raise ImportError("catboost is required for catboost_ranker inference")
            model = CatBoost()
            model.load_model(model_dir / f"catboost_ranker_{target_col}_fold{fold}.txt")
            return model
        if model_type == "xgb":
            if xgb is None:
                raise ImportError("xgboost is required for xgb inference")
            model = xgb.Booster()
