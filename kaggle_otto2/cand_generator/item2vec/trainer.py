...
class Item2VecTrainer:
    def __init__(self, data_loader: OttoDataLoader, config: Config):
        ...
        self.lr = self.config.yaml.cg.item2vec.get("lr", 0.1)
        self.window = self.config.yaml.cg.item2vec.get("window", 5)
        self.sg = self.config.yaml.cg.item2vec.get("sg", 1)
        self.negative = self.config.yaml.cg.item2vec.get("negative", 20)
        self.min_count = self.config.yaml.cg.item2vec.get("min_count", 1)
