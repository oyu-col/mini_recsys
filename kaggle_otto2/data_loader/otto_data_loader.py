...
class OttoDataLoader:
    ...
    def _resolve_input_dir(self):
        if self.config.is_cv:
            env_var = "OTTO_CV_DATASET_DIR"
            default_names = [
                "otto-train-and-test-data-for-local-validation",
                "otto-train-and-validation-extracted-from-train",
            ]
            required_files = ["train.parquet", "test.parquet", "test_labels.parquet"]
        else:
            env_var = "OTTO_LB_DATASET_DIR"
            default_names = ["otto-full-optimized-memory-footprint"]
            required_files = ["train.parquet", "test.parquet"]

        env_path = os.environ.get(env_var)
        if env_path:
            candidate = self.config.dir_config.input_dir / env_path
            if not candidate.exists():
                candidate = Path(env_path)
            if self._has_required_files(candidate, required_files):
                return candidate
            raise FileNotFoundError(
                f"{env_var}={env_path} does not contain {required_files}"
            )

        for name in default_names:
            candidate = self.config.dir_config.input_dir / name
            if self._has_required_files(candidate, required_files):
                return candidate

        for candidate in self.config.dir_config.input_dir.glob("*"):
            if candidate.is_dir() and self._has_required_files(candidate, required_files):
                return candidate

        raise FileNotFoundError(
            "Could not find OTTO parquet dataset under "
            f"{self.config.dir_config.input_dir}. Expected files: {required_files}. "
            f"You can set {env_var} to the dataset directory path."
        )

    @staticmethod
    def _has_required_files(candidate, required_files):
        return candidate.exists() and all((candidate / f).exists() for f in required_files)
